...
TaskHandle_t sw7_debounce_task_handle = NULL;
 ...
 */
 void sw7_debounce_task(void* unused_arg) {
    uint32_t sw_state = 1;            // initialize sw7_state
    uint32_t sw_previous_state = 1;   // initialize sw7_previous_state
    uint32_t count = 5;               // initialize sw7_final_state
    uint8_t pcfbuffer[]={0b11111111,0b11111111};// data buffer, must be two bytes
    
    xQueueOverwrite(xQsw7, &sw_state);
    while (true) {
 ...
 */
 void sw_debounce_task(void* unused_arg) {
    uint32_t sw_state = 1;            // initialize sw_state
    uint32_t sw_previous_state = 1;   // initialize sw_previous_state
...
int main() {
    ...
    BaseType_t sw7_status = xTaskCreate(sw7_debounce_task, 
                                         "SW7_DEBOUNCE_TASK", 
                                         256, 
                                         NULL, 
                                         5,     // Task priority
                                         &sw7_debounce_task_handle);
        if (sw7_status != pdPASS) {
           error_state  += 1;
            }
    ...
    xQsw = xQueueCreate(1, sizeof(uint32_t)); 
    if ( xQsw == NULL ) error_state += 1;

    xQsw0 = xQueueCreate(1, sizeof(uint32_t)); 

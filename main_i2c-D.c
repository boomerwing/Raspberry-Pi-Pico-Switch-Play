...
void blink_task(void* unused_arg) {
   ...
   // Initialize start time for vTaskDelayUntil
    ...
    while (true) {
        // SW0 changes blink rate
        uxMessagesWaiting = uxQueueMessagesWaiting(xQsw0);
        if(uxQueueMessagesWaiting){
            xQueuePeek(xQsw0, &sw_buffer,0);
            }
         if(sw_buffer) steps = 900;
         else steps = 450;  
...
void gpio_led_task(void* unused_arg) {  
    ...
    while (true) {
        ...
        if(uxQueueMessagesWaiting){
            ...
            gpio_put(D6_PIN, blink_buffer == 0 ? 0 : 1);
            gpio_put(D7_PIN, blink_buffer == 1 ? 0 : 1);
        }
 ...
 * the Switch state is passed to a function to be used as a control signal.
 */

...
void gpio_led_task(void* unused_arg) {  
    ...
    while (true) {
        ...
        if(uxQueueMessagesWaiting(xQbin3)){  // output of SW 2,3,4 Binary 0Bxxx
            xQueuePeek(xQbin3, &swbin_buffer,0);
        }
        ...
        if( swbin_buffer == 0 ) gpio_put(G16, 1);
        else gpio_put(G16,0);
 ...
 */
void switchBin2(uint8_t sw_state) {
    uint8_t now = 0 ;
    
    now = (sw_state & 0B0000011);
...
void configure_gpio(void) {
    ...
    // Configure D21_PIN for led_task_gpio
    gpio_init(G16);
    gpio_disable_pulls(G16);  // remove pullup and pulldowns
    gpio_set_dir(G16, GPIO_OUT);

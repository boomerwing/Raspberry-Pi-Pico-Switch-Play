...
volatile QueueHandle_t xQsw07 = NULL;
...
volatile QueueHandle_t xQsw = NULL;
...
TaskHandle_t sw07_debounce_task_handle = NULL;
...
void adc_task(void* unused_arg) {
    ...
    uint32_t ack_flag = 0;
    uint32_t measured = 1;
    uint32_t sw7_state = 0;
    uint32_t sw7_buffer = 0;
...
//        if(measured < ALARM_BOUNDARY) {      // ** blink drive  **
        if(now < HEX_ALARM_BOUNDARY) {      // ** blink drive  **
            ...
            if(ack_flag != 0) {   // ****  Blink until Ack seen  ***
                ...
                uxMessagesWaiting = uxQueueMessagesWaiting(xQsw7);
 ...
 * as a control signal, passed to an action function by a Queue.
 ...
 void sw07_debounce_task(void* unused_arg) {
    uint8_t sw_previous_state = 1;   // initialize sw7_previous_state
    uint8_t sw_state = 1;            // initialize sw7_state
    uint32_t count = 5;               // initialize sw7_final_state
    uint8_t pcfbuffer[]={0b11111111,0b11111111};// data buffer, must be two bytes
    
    xQueueOverwrite(xQsw07, &sw_state);
    while (true) {
        ...
        sw_state = readBit(pcfbuffer[0],SW7);
        ...
        vTaskDelay(ms_delay60);  // check switch state every 50 ms
    }  // End while (true)    
}
*/
...
/*
 * Switch0(int)
 *  Input Switch state value.  The function will check the specific Switch
 * for change, and if it is changed, will send Qmessage to task requuiring
 * switch change.
 */
 ...
 * Switch1(int)
 *  Input Switch state value.  The function will check the specific Switch
 * for change, and if it is changed, will send Qmessage to task requiring
 * switch change.
 */
...
/**            
    BaseType_t sw07_status = xTaskCreate(sw07_debounce_task, 
                                         "SW07_DEBOUNCE_TASK", 
                                         256, 
                                         NULL, 
                                         4,     // Task priority
                                         &sw07_debounce_task_handle);
        if (sw07_status != pdPASS) {
           error_state  += 1;
            }
 */          
    ...
    xQsw = xQueueCreate(1, sizeof(uint8_t)); 
    if ( xQsw == NULL ) error_state += 1;
    ...
    xQsw07 = xQueueCreate(1, sizeof(uint8_t)); 
    if ( xQsw07 == NULL ) error_state += 1;
